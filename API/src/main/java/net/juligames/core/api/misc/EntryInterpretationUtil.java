...
public class EntryInterpretationUtil {
    ...
    @Contract("_, _, _ -> new")
    public static <K, V> Map.@NotNull @Unmodifiable Entry<String, String> reverseEntry(Map.@NotNull Entry<K, V> stringStringEntry, @NotNull Interpreter<K> kInterpreter, @NotNull Interpreter<V> vInterpreter) throws Exception {
    ...
    public static <K, V> @Unmodifiable Collection<Map.Entry<String, String>> reverseEntries(@NotNull Collection<Map.Entry<K,V>> collection, Interpreter<K> kInterpreter, Interpreter<V> vInterpreter) {
        return collection.stream().map(kvEntry -> {
            try {
                return reverseEntry(kvEntry,kInterpreter,vInterpreter);
            } catch (Exception e) {
